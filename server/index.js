...
const app = express();
app.use(cors());
app.use(express.json({ limit: '10mb' })); // Increase limit for large context
...
const openai = OPENROUTER_KEY ? new OpenAI({
    ...
    // dangerouslyAllowBrowser: true, // Not needed in Node
    ...
    try {
        ...
        **預計篇幅：${totalChapters} 章**。
        ...
        if (OPENROUTER_KEY && shouldCallDeepSeek) {
            result = await callDeepSeek("你是一位腦洞大開的頂級網文創意總監。", prompt, true, 0.9);
        } else {
            const model = getGeminiModel(true);
            const r = await model.generateContent(prompt);
            result = cleanJson(r.response.text());
        }
        res.json(result);
        ...
        if (isGeminiBlockedError(error)) {
            try {
                ...
                try {
                    res.json(cleanJson(fallbackStr));
                } catch (e) {
                    res.json({ title: "生成失敗", summary: "格式錯誤", protagonist: {}, loveInterest: {} });
                }
            } catch (e) {
                res.json({ title: "生成失敗", summary: "請重試。", protagonist: {}, loveInterest: {} });
            }
        } else {
            res.status(500).json({
                title: "生成失敗",
...
app.post('/api/ensure-detailed-settings', async (req, res) => {
    try {
        const { genre, simpleSettings, tags, tone, category, useDeepSeek } = req.body;
        const toneDesc = getToneInstruction(tone);
        const styleGuide = `風格標籤：${tags.join('、')}。\n${toneDesc}`;
        ...
        let result;
        if (OPENROUTER_KEY && useDeepSeek) {
            result = await callDeepSeek("你是一位專業的小說架構師。", prompt, true);
        } else {
            const model = getGeminiModel(true);
            const r = await model.generateContent(prompt);
            result = cleanJson(r.response.text());
        ...
        res.json({
            design_blueprint: {},
            protagonist: { name: req.body.simpleSettings.protagonist, gender: "未知", profile: {} },
            loveInterest: { name: req.body.simpleSettings.loveInterest, gender: "未知", profile: {} }
        });
    }
});
...
app.post('/api/refine-character', async (req, res) => {
    try {
        const { charInfo, novelContext, useDeepSeek } = req.body;
        const prompt = `
        ...
        let result;
        if (OPENROUTER_KEY && useDeepSeek) {
            result = await callDeepSeek("你是一位專業的角色設計師。", prompt, true);
        } else {
            const model = getGeminiModel(true);
            const r = await model.generateContent(prompt);
            result = cleanJson(r.response.text());
        ...
        res.json({
            name: req.body.charInfo.name,
            role: req.body.charInfo.role,
            gender: req.body.charInfo.gender,
            description: req.body.charInfo.description,
            profile: {}
        });
    }
});
...
app.post('/api/generate-start', async (req, res) => {
    try {
        ...
        if (OPENROUTER_KEY && useDeepSeek) {
            const result = await callDeepSeek(systemPrompt, userPrompt, true);
            res.json(result);
        } else {
            const model = getGeminiModel(true);
        ...
        if (isGeminiBlockedError(error) || error.message.includes("DeepSeek")) {
            try {
                ...
                res.json({ content: content, character_updates: [] });
            } catch (e) {
                res.status(500).json({ error: "生成失敗，請重試" });
            }
        } else {
    ...
    try {
        ...
        if (isGeminiBlockedError(error)) {
            ...
            try {
                ...
                // Re-calculate director for fallback context
                const { novelContext, prevText, tags, pov, lastPlotState } = req.body;
                const totalChapters = novelContext.targetEndingChapter || getRecommendedTotalChapters(novelContext.genre);
                const director = determinePlotDirectives(novelContext.currentChapterIndex, lastPlotState, novelContext.genre, tags, totalChapters);
                const baseSystemPrompt = `你是一名專業的小說家。請撰寫下一章並維護世界觀數據。`;
...
Task: Write next chapter.
                ...
                res.status(500).json({ error: "系統暫時無法生成內容，請稍後再試。" });
            }
        } else {
            res.status(500).json({ error: error.message });
        }
