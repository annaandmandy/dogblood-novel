...
${ANTI_CLICHE_INSTRUCTIONS}
【無限流特化禁令】
1. **場景真實感**：主神空間/副本場景必須有質感（氣味、光影、髒污），不要寫成虛擬的數據流。
2. **危機的具象化**：不要只說「很危險」。請描寫怪物滴落的涎水、槍口的冰冷、隊友被抹殺時的無聲崩解。
3. **拒絕數據刷屏**：除非必要，不要大段列出屬性面板。
`;
...
export const generateInfiniteSettings = async (tags = [], tone = "一般", targetChapterCount = null, category = "BG", useDeepSeek = false) => {
    ...
    const dungeonRequirement = isRuleBased
        ? "設計一個【規則怪談】類型的副本。必須包含5-8條詭異的紅藍字規則，以及規則背後的邏輯陷阱。"
        : "設計一個【生存/動作/解謎】類型的副本。重點在於「主線任務」與「環境威脅」。不需要複雜的紙條規則，重點是通關條件。";
    ...
    **類別**：${category}。**篇幅**：${totalChapters} 章。
    ...
    2. **主線謎題**：主角進入無限世界並非偶然，請設計一個貫穿全書的懸疑主線。
    ...
    【回傳 JSON】
    {
      ...
      "loveInterest": { "name": "對象名", "role": "...", "gender": "...", "profile": { "appearance": "...", "personality_surface": "...", "personality_core": "...", "biography": "...", "trauma": "...", "desire": "..." } }
    }
    `;
...
export const generateInfiniteStart = async (settings, tags = [], tone = "一般", pov = "女主", useDeepSeek = false) => {
    ...
    let mechanismDisplay = "";
    if (isRuleBased) {
        const rules = firstDungeon?.core_rules?.join('\n') || "未知規則";
        mechanismDisplay = `**規則展示**：主角發現了寫有詭異規則的紙條/告示牌：\n${rules}\n請描寫主角閱讀規則時的心理壓力。`;
    } else {
        const missions = firstDungeon?.missions?.join('\n') || "任務：存活";
        mechanismDisplay = `**任務發布**：系統/主神冰冷的聲音（或視網膜上的文字）發布了本次副本任務：\n${missions}\n請描寫主角接到任務後的緊迫感與生存壓力。`;
    }
    ...
    【回傳 JSON】
    {
      ...
      "plot_state": {
          ...
          "instance_progress": 5,
          "cycle_num": 1,
          "current_dungeon": null, // Placeholder, will be overwritten
          "current_rules": null // Placeholder
      }
    }
    `;
